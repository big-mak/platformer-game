...
while run:
    ...
    if main_menu:  # stays on menu screen
        if start_button.draw():
            ...
            if level == 8:                # for use in custom level
            ...
            if path.exists(f"level{level}_data"):       # checks if level exists, then load it
                pickle_in = open(f"level{level}_data", "rb")
                world_data = pickle.load(pickle_in)
                world = World(world_data)
            else:
                tk.messagebox.showinfo(title="Error", message="No level found", )  # returns error message
            ...
            if type(level) != int:
                ...
                if level <= max_levels:
                    ...
                    if type(level) == int and save_counter == 0:
                        if save_button.draw():
                            f = open("score_data", "rb")  # opens list of scores to add to it
                            data = f.read()
                            data2 = pickle.loads(data)
                            f.close()
                            pickle_out = open("score_data", "wb")
                            SCORE_INP = simpledialog.askstring(title="Save score", prompt="Enter name to save score")
                            data2.append([SCORE_INP, total_score])  # adds team name and respective score
                            pickle.dump(data2, pickle_out)
                            pickle_out.close()
                            save_counter += 1  # makes the save button disappear afterwards
