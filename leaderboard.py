...
def leader():
    ...
    leaderboard_data.reverse()           # orders leaderboard

    root = Tk()
    scrollbar = Scrollbar(root)
    scrollbar.pack(side=RIGHT, fill=Y)     # creates a scrollbar window for the leaderboard

    my_list = Listbox(root, yscrollcommand=scrollbar.set)
    for i in leaderboard_data:
        my_list.insert(END, str(i))        # adds data into scrollbar window
