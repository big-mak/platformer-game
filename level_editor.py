def level_editor():
    ...
    pickle_in = open(f"level111_data", "rb")
    ...
    while run:
        ...
        if pygame.mouse.get_pressed()[0] == 1:    # mouse has been clicked
            pos = pygame.mouse.get_pos()
            x1 = pos[0]//30                       # gets x co-ordinate of mouse
            y1 = pos[1]//30                       # gets y co-ordinate of mouse
            if 0 < x1 < 19 and 0 < y1 < 19:       # doesnt let user change outer blocks
                world_data[y1][x1] = (world_data[y1][x1] + 1) % 9  # cycles through blocks
        ...
        if key[pygame.K_s]:
            ...
            pickle_out.close()
            tk.messagebox.showinfo(title="Saved", message="Level saved successfully", )  # level has been saved
            run = False
